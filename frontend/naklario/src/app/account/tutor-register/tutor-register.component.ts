...
export class TutorRegisterComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.constants$ = this.activatedRoute.data.pipe(map((data) => {
    ...
    this.constants$.pipe(first()).subscribe(
      (c: Constants) => {
        const data: SchoolData[][] = [];
        for (const schoolType of c.schoolTypes) {
          data.push(
            c.schoolData.filter((x) => x.schoolType.id === schoolType.id)
          );
        }
    ...
    combineLatest([this.constants$, this.verificationFile$]).subscribe(
      ([c, file]) => {
        ...
        const tutorData: SendableTutorData = {
          schooldata: grades,
          subjects: this.subjectsGroup.value.subjects.filter((x: boolean) => x === true).map((_, i) => c.subjects[i].id),
